...
class MotionCalculator {
  public:
    void run() {
        char choice;
        cout
            << "What do you want to calculate? (d for distance, s for speed, t "
               "for time): ";
        cin >> choice;
  ...
  private:
    ...
    double calculateDistance(double speed, double time, const string &distUnit,
                             const string &speedUnit, const string &timeUnit) {
        double speedBase =
            toMeters(speed, speedUnit.substr(0, speedUnit.find('/'))) /
            toSeconds(1, speedUnit.substr(speedUnit.find('/') + 1));
    ...
    double calculateSpeed(double distance, double time, const string &distUnit,
                          const string &timeUnit) {
        double distBase = toMeters(distance, distUnit);
        double timeBase = toSeconds(time, timeUnit);
        double speedBase = distBase / timeBase; // m/s
        // return in a standard unit like m/s or km/h, return to m/s for
        // simplicity
        return speedBase;
    }

    // calculating time in base units, then convert back
    double calculateTime(double distance, double speed, const string &distUnit,
                         const string &speedUnit, const string &timeUnit) {
        double distBase = toMeters(distance, distUnit);
        double speedBase =
            toMeters(speed, speedUnit.substr(0, speedUnit.find('/'))) /
